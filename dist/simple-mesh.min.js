...
function Face(opts){_classCallCheck(this,Face);if(opts==undefined)throw new Error('You must pass an `opts` argument to the Face constructor');this.id=opts.id;this.vertices=opts.vertices;this.fillStyle=opts.fillStyle}_createClass(Face,[{key:'draw',value:function draw(ctx,style,origin){// Plot the shape
...
function Vertex(opts){_classCallCheck(this,Vertex);if(opts==undefined)throw new Error('You must pass an `opts` argument to the Vertex constructor');if(opts.hasOwnProperty('id')==false)throw new Error('`id` is required');if(opts.hasOwnProperty('x')==false)throw new Error('`x` is required');if(opts.hasOwnProperty('y')==false)throw new Error('`y` is required');if(opts.hasOwnProperty('z')==false)throw new Error('`z` is required');this.id=opts.id;this.oX=opts.x;this.oY=opts.y;this.oZ=opts.z;this.x=opts.x;this.y=opts.y;this.z=opts.z}_createClass(Vertex,[{key:'draw',value:function draw(ctx,style){// Draw the circle
ctx.beginPath();ctx.arc(this.x,this.y,style.radius,0,2*Math.PI,false);ctx.closePath();// Style the circle
ctx.lineWidth=style.lineWidth;if(ctx.fillStyle=style.fillStyle)ctx.fill();if(ctx.strokeStyle=style.strokeStyle)ctx.stroke();// Draw the label
if(style.labelVisibility){var x=this.x+style.labelOffset.x;var y=this.y-style.labelOffset.y;ctx.lineWidth=style.labelLineWidth;if(ctx.strokeStyle=style.labelStrokeStyle)ctx.strokeText(this.id,x,y);if(ctx.fillStyle=style.labelFillStyle)ctx.fillText(this.id,x,y)}}}]);return Vertex}();// Export on server only
