...
function SimpleMesh(context,opts){_classCallCheck(this,SimpleMesh);if(!context)throw new Error('A `context` argument is required');if(!opts)throw new Error('A `opts` argument is required');var defaults={fog:false,origin:{x:0,y:0,z:0},theta:{x:0,y:0,z:0},// Rotation angles in +/- degrees
...
this.validate();return this}_createClass(SimpleMesh,[{key:'deepExtend',value:function deepExtend(){for(var i=arguments.length-1;i>0;i--){var source=arguments[i];var dest=arguments[i-1];for(var prop in source){if(_typeof(source[prop])=='object'&&!Array.isArray(source[prop])){if(!dest.hasOwnProperty(prop))dest[prop]={};dest[prop]=this.deepExtend(dest[prop],source[prop])}else{dest[prop]=source[prop]}}}return arguments[0]}},{key:'validate',/* Validate properties */value:function validate(){if(typeof this.fog!='boolean')throw new Error('The `fog` property must be boolean');if(_typeof(this.origin)!='object')throw new Error('The `origin` property must be an object');if(typeof this.origin.x!='number')throw new Error('The `origin.x` property must be a number');if(typeof this.origin.y!='number')throw new Error('The `origin.y` property must be a number');if(typeof this.origin.z!='number')throw new Error('The `origin.z` property must be a number');if(_typeof(this.theta)!='object')throw new Error('The `theta` property must be an object');if(typeof this.theta.x!='number')throw new Error('The `theta.x` property must be a number');if(typeof this.theta.y!='number')throw new Error('The `theta.y` property must be a number');if(typeof this.theta.z!='number')throw new Error('The `theta.z` property must be a number');if(typeof this.vertexVisibility!='boolean')throw new Error('The `vertexVisibility` property must be boolean');if(_typeof(this.vertexStyle)!='object')throw new Error('The `vertexStyle` property must be an object');if(typeof this.edgeVisibility!='boolean')throw new Error('The `edgeVisibility` property must be boolean');if(_typeof(this.edgeStyle)!='object')throw new Error('The `edgeStyle` property must be an object');if(typeof this.faceVisibility!='boolean')throw new Error('The `faceVisibility` property must be boolean');if(_typeof(this.faceStyle)!='object')throw new Error('The `faceStyle` property must be an object')}/* Load objects */},{key:'loadObjects',value:function loadObjects(opts){// Vertices
...
if(this.vertices[edge.a]=='undefined')throw new Error('A is not a vertex');if(this.vertices[edge.b]=='undefined')throw new Error('B is not a vertex');edge.id=_id;edge.a=this.vertices[edge.a];edge.b=this.vertices[edge.b];//
this.edges[_id]=new Edge(edge)}}// Faces
if(opts.hasOwnProperty('faces')){for(var faceId in opts.faces){var face=opts.faces[faceId];face.id=faceId;for(var vertexId in face.vertices){var vertex=face.vertices[vertexId];face.vertices[vertexId]=this.vertices[vertex]}this.faces[faceId]=new Face(face)}}}},{key:'loadArrays',/* Load simple arrays */value:function loadArrays(obj){if(obj.vertices==null)throw new Error('An array of vertices must is required');var verticeOrder=[];// Store the vertex order for loading edges
...
if(typeof factor!='number')throw new Error('Factor "'+factor+'" must be numeric');// Loop over the vertices and multiply
...
var axes=['x','y','z'];if(axes.indexOf(a)<0)throw new Error('Axis A must be one of x, y, or z');if(axes.indexOf(b)<0)throw new Error('Axis B must be one of x, y, or z');if(typeof theta!='number')throw new Error('Theta must be numeric');// Perform a matrix rotation
